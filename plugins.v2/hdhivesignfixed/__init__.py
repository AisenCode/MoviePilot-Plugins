...
class HdhiveSignFixed(_PluginBase):
    # 插件名称
    plugin_name = "影巢签到修复版"
    # 插件描述
    ...
    # 插件作者
    plugin_author = "madrays,EchoCC"
    # 作者主页
    author_url = "https://github.com/madrays"
    # 插件配置项ID前缀
    plugin_config_prefix = "hdhivesignfixed_"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        logger.info("============= hdhivesignfixed 初始化 =============")
        try:
            if config:
                self._enabled = config.get("enabled")
                self._cookie = config.get("cookie")
                self._notify = config.get("notify")
                self._degflag = config.get("degflag", False)
                ...
                logger.info(f"影巢签到插件已加载，配置：enabled={self._enabled}, notify={self._notify}, degflag={self._degflag}, cron={self._cron}")
            ...
            logger.error(f"hdhivesignfixed初始化错误: {str(e)}", exc_info=True)
    ...
    def _signin_base(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 动态获取 next-action
            next_action = self._get_next_action_cached(cookies, token)

            proxies = settings.PROXY
            ua = settings.USER_AGENT
            headers = {
                'User-Agent': ua,
                'Accept': 'aptext/x-component',
                ...
                if response_text:
                    ...
                    for line in lines:
                        ...
                        if not line.strip():
                            continue
                        # 查找以数字开头后跟冒号的行
                        if ':' in line and line.strip()[0].isdigit() and 'code' in line:
                            # 提取冒号后的JSON部分
                            json_part = line.split(':', 1)[1].strip()
                            if json_part.startswith('{') and json_part.endswith('}'):
                                json_line = json_part
                                break
                    logger.debug(f"json_line: {str(json_line)}")
                    if json_line:
                        parsed_json = json.loads(json_line)
                        # 处理新的响应格式，检查是否有response或error包装
                        if 'response' in parsed_json:
                            signin_result = parsed_json['response']
                        elif 'error' in parsed_json:
                            signin_result = parsed_json['error']
                        else:
                            signin_result = parsed_json
                    else:
                        # 如果没找到带数字前缀的JSON行，尝试直接解析整个响应
            ...
            if "已经签到" in description or "签到过" in description :
                try:
                    self._fetch_user_info(cookies, token)
                except Exception:
                    pass
            ...
            logger.error(f"签到失败, HTTP状态码: {signin_res.status_code}, 消息: {message +'，' + description}")
                ...
                try:
                    ...
                    if js_response.status_code == 200:
                        ...
                        if matches:
                            ...
                            logger.info(f"动态获取到next-action: {next_action}")
                            return next_action
                            
                except Exception as e:
                    logger.debug(f"获取JS文件失败 {js_url}: {str(e)}")
                    continue
            
            logger.warning("未找到checkIn相关的next-action，使用默认值")
            return defuault_next_action
            
        except Exception as e:
            logger.error(f"动态获取next-action失败: {str(e)}")
            return defuault_next_action

    def _get_next_action_cached(self, cookies, token):
        """带缓存的动态获取 next-action"""
        import time
        
        # 检查缓存（7天有效）
        if hasattr(self, '_next_action_cache') and hasattr(self, '_next_action_cache_time'):
            if time.time() - self._next_action_cache_time < 60*60*24*7:
                logger.debug(f"使用缓存的next-action: {self._next_action_cache}")
    ...
    def _fetch_user_info(self, cookies: Dict[str, str], token: str) -> Optional[dict]:
        try:
            ...
            info = {
                ...
                'avatar_url': detail.get('avatar_url') or detail.get('gravatar_url') or f"{self._base_url}/images/default-avatar.webp",
            ...
            if not info.get('nickname') or info.get('points') is None or info.get('signin_days_total') is None:
                try:
                    ...
                    logger.debug(f"RSC 请求地址：{rsc_url},headers: {str(rsc_headers)},返回：{rsc_text}")
                    ...
                    if (not info.get('nickname') or info.get('points') is None or info.get('signin_days_total') is None) and '"user":' in rsc_text:
                        ...
                        if user_json:
                            try:
                                ...
                                info['avatar_url'] = obj.get('avatar_url') or info.get('avatar_url') or f"{self._base_url}/images/default-avatar.webp"
    ...
    def _send_sign_notification(self, sign_dict):
        ...
        degflag = sign_dict.get("degflag", False)
    ...
    def get_state(self) -> bool:
        logger.info(f"hdhivesignfixed状态: {self._enabled}")
        ...
        for history in historys:
            ...
            history_rows.append({
                ...
                'content': [
                    ...
                    {'component': 'td', 'text': '是' if history.get('degflag', False) else '否'},
